  ...
  cal = new Calendar('#calendar', {
    ...
    template: {
      ...
      time: function (schedule) {
           ...
           return '<span style="position: absolute;padding: 0;width: 8px;height: 8px;top: 6px;left: 0;border-radius: 50%;background:'+schedule.color+';"></span><span   style="background-color:' + schedule.bgColor + '">' + schedule.title + '</span>';  
  ...
  function getTimeTemplate(schedule, isAllDay) {
    ...
    if (schedule.isPrivate) {
      ...
      if (schedule.isReadOnly) {
        ...
        //html.push('<span class="bx bxs-location font-size-small align-middle"></span>');
       //} 
...
//   }
 ...
 // setSchedules();
  function setSchedules() {
      var meeting_current_status="all";
    cal.clear();
    //generateSchedule(cal.getViewName(), cal.getDateRangeStart(), cal.getDateRangeEnd());
    $.post("../controller/schedule/schedule_calendar.php",{action:'display_all_schedule',v_meeting_status:meeting_current_status},function(result){
      var obj = JSON.parse(result);
      var x =   JSON.stringify(obj, null, "\t"); // stringify with tabs inserted at each level
       x =  JSON.stringify(obj, null, 4);    // stringify with 4 spaces at each level
      
      
      console.log(x);
      
    
       cal.createSchedules(obj);
    });
...
(function () {
  ...
  CalendarList.forEach(function (calendar) {
    html.push('<div class="sidebar-calendars-item"><label>' +
      '<input type="checkbox" class="tui-full-calendar-checkbox-round" name="meeting" id="meeting_status"  value="' + calendar.name + '" checked>' +
